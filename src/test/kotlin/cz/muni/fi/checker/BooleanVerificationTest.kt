...
class AndVerificationTest {
    ...
    fun simpleTest() {
        ...
        val checker = ModelChecker(model, createSharedMemoryJobQueue<IDNode, IDColors>( 1 ).first())
    ...
    fun nestedConcurrentTest() {
        ...
        val result = listOf(0..1667, 1668..4232, 4232..8000).map { bounds ->
            RegularKripkeFragment(bounds)
        }.zip(createSharedMemoryJobQueue<IDNode, IDColors>(3)).map {
            ModelChecker(it.first, it.second)
        }.map { it.verify(p1 and p2 and p3) }.fold(emptyIDNodes) { r, l ->
            r union l
        }
    ...
    fun simpleConcurrentTest() {
        ...
        val result = listOf(0..1867, 1868..4000).map { bounds ->
            RegularKripkeFragment(bounds)
        }.zip(createSharedMemoryJobQueue<IDNode, IDColors>(2)).map {
            ModelChecker(it.first, it.second)
        }.map { it.verify(p1 and p2) }.fold(emptyIDNodes) { r, l ->
            r union l
        }
...
class OrVerificationTest {
    ...
    fun simpleConcurrentTest() {
        ...
        val result = listOf(0..1896,1897..4000).map {
    ...
    fun nestedConcurrentTest() {
        ...
        val result = listOf(0..1896,1897..3975, 3976..8000).map {
            RegularKripkeFragment(it)
        }.zip(createSharedMemoryJobQueue<IDNode, IDColors>(3)).map {
            ModelChecker(it.first, it.second)
        }.map {
